...
def load_data(dataset_name="wikiann"):
...
def main(config=None):
    ...
    with wandb.init(config=config):  # Initialisation of wandb
        ...
        dataset, label_names = load_data(dataset_name="wikiann") # Loading dataset
        ...
        for epoch in range(args.epochs):
            ...
            if valid_loss < best_valid_loss:
                ...
                torch.save(model.state_dict(), 'tut6-model-slot.pt')
...
if __name__ == "__main__":
    ...
    sweep_id = wandb.sweep(sweep_config, project="ner-sweep-slot-attention-mask-wikiann")
    # main(args)
